

function setStateInfo(str){
    var obj = [];
    if (str == "Alabama"){ //OK
        obj.push(32.7990);
        obj.push(-86.8073);
        obj.push(6);
    } else if(str == "Alaska"){ //OK
        obj.push(64.4728);
        obj.push(-152.2683);
        obj.push(3);
    } else if(str == "Arizona"){ // OK
        obj.push(34.2527);
        obj.push(-111.3877);
        obj.push(6);
    } else if(str == "Arkansas"){ //OK
        obj.push(34.7958);
        obj.push(-92.3809);
        obj.push(7);
    } else if(str == "California"){ //OK
        obj.push(37.1700);
        obj.push(-119.7462);
        obj.push(5);
    } else if(str == "Colorado"){ //OK
        obj.push(39.0646);
        obj.push(-105.3272);
        obj.push(6);
    } else if(str == "Connecticut"){ //OK
        obj.push(41.5250);
        obj.push(-72.7622);
        obj.push(8);
    } else if(str == "Delaware"){ //OK
        obj.push(39.1441);
        obj.push(-75.5148);
        obj.push(8);
    } else if(str == "District Of Columbia"){ //OK
        obj.push(38.8964);
        obj.push(-77.0262);
        obj.push(11);
    } else if(str == "Florida"){ //OK
        obj.push(27.8333);
        obj.push(-83.4521);
        obj.push(6);
    } else if(str == "Georgia"){ //OK
        obj.push(32.9866);
        obj.push(-83.6487);
        obj.push(6);
    } else if(str == "Hawaii"){ //OK
        obj.push(22.1098);
        obj.push(-167.5311);
        obj.push(5);
    } else if(str == "Idaho"){ //OK
        obj.push(45.2394);
        obj.push(-114.5103);
        obj.push(5);
    } else if(str == "Illinois"){ //OK
        obj.push(39.9771);
        obj.push(-89.0022);
        obj.push(6);
    } else if(str == "Indiana"){ //OK
        obj.push(39.8647);
        obj.push(-86.2604);
        obj.push(6);
    } else if(str == "Iowa"){ //OK
        obj.push(42.0046);
        obj.push(-93.7140);
        obj.push(7);
    } else if(str == "Kansas"){ //OK
        obj.push(38.5111);
        obj.push(-98.6005);
        obj.push(7);
    } else if(str == "Kentucky"){ //OK
        obj.push(37.6690);
        obj.push(-86.1514);
        obj.push(7);
    } else if(str == "Louisiana"){ //OK
        obj.push(31.1801);
        obj.push(-91.8749);
        obj.push(6);
    } else if(str == "Maine"){ //OK
        obj.push(45.2074);
        obj.push(-69.3977);
        obj.push(6);
    } else if(str == "Maryland"){ //OK
        obj.push(38.7724);
        obj.push(-76.9902);
        obj.push(7);
    } else if(str == "Massachusetts"){ //OK
        obj.push(42.0373);
        obj.push(-71.5314);
        obj.push(7);
    } else if(str == "Michigan"){
        obj.push(45.0504);
        obj.push(-84.5603);
        obj.push(6);
    } else if(str == "Minnesota"){ //OK
        obj.push(46.4326);
        obj.push(-93.9196);
        obj.push(6);
    } else if(str == "Mississippi"){
        obj.push(40.7673);
        obj.push(-89.6812);
        obj.push(6);
    } else if(str == "Missouri"){ //OK
        obj.push(38.4623);
        obj.push(-92.8020);
        obj.push(6);
    } else if(str == "Montana"){ //OK
        obj.push(46.7326);
        obj.push(-110.5196);
        obj.push(6);
    } else if(str == "Nebraska"){ //OK
        obj.push(41.4289);
        obj.push(-100.3883);
        obj.push(6);
    } else if(str == "Nevada"){ //OK
        obj.push(38.6199);
        obj.push(-117.1219);
        obj.push(6);
    } else if(str == "New Hampshire"){ //OK
        obj.push(43.9108);
        obj.push(-71.5653);
        obj.push(7);
    } else if(str == "New Jersey"){ //OK
        obj.push(40.1140);
        obj.push(-74.5089);
        obj.push(7);
    } else if(str == "New Mexico"){ //OK
        obj.push(34.4375);
        obj.push(-106.2371);
        obj.push(6);
    } else if(str == "New York"){ //OK
        obj.push(42.9497);
        obj.push(-75.9384);
        obj.push(6);
    } else if(str == "North Carolina"){ //OK
        obj.push(35.4411);
        obj.push(-79.8431);
        obj.push(6);
    } else if(str == "North Dakota"){ //OK
        obj.push(47.5362);
        obj.push(-100.2930);
        obj.push(6);
    } else if(str == "Ohio"){ //OK
        obj.push(40.3736);
        obj.push(-82.7755);
        obj.push(6);
    } else if(str == "Oklahoma"){ //OK
        obj.push(35.5376);
        obj.push(-97.9247);
        obj.push(6);
    } else if(str == "Oregon"){ //OK
        obj.push(43.8978);
        obj.push(-120.6519);
        obj.push(6);
    } else if(str == "Pennsylvania"){ //OK
        obj.push(40.9773);
        obj.push(-77.9640);
        obj.push(7);
    } else if(str == "Rhode Island"){ //OK
        obj.push(41.6772);
        obj.push(-71.5101);
        obj.push(8);
    } else if(str == "South Carolina"){ //OK
        obj.push(33.6191);
        obj.push(-80.9066);
        obj.push(7);
    } else if(str == "South Dakota"){ //OK
        obj.push(44.2853);
        obj.push(-100.4632);
        obj.push(6);
    } else if(str == "Tennessee"){ //OK
        obj.push(35.7449);
        obj.push(-86.7489);
        obj.push(6);
    } else if(str == "Texas"){ //OK
        obj.push(31.1060);
        obj.push(-99.6475);
        obj.push(5);
    } else if(str == "Utah"){ //OK
        obj.push(39.5135);
        obj.push(-111.8535);
        obj.push(6);
    } else if(str == "Vermont"){ //OK
        obj.push(43.8407);
        obj.push(-72.7093);
        obj.push(7);
    } else if(str == "Virginia"){ //OK
        obj.push(37.9680);
        obj.push(-79.5057);
        obj.push(7);
    } else if(str == "Washington"){ //OK
        obj.push(47.3537);
        obj.push(-120.4101);
        obj.push(6);
    } else if(str == "West Virginia"){ //OK
        obj.push(38.8680);
        obj.push(-80.9696);
        obj.push(7);
    } else if(str == "Wisconsin"){ //OK
        obj.push(44.7563);
        obj.push(-89.6385);
        obj.push(6);
    } else if(str == "Wyoming"){ //OK
        obj.push(42.9475);
        obj.push(-107.2085);
        obj.push(6);
    } else if(str == "American Samoa"){
        obj.push(-14.2417);
        obj.push(-170.0397);
        obj.push(8);
    } else if(str == "Northern Mariana Islands"){
        obj.push(17.08058);
        obj.push(145.5505);
        obj.push(6);
    } else if(str == "Guam"){
        obj.push(13.4658);
        obj.push(144.7505);
        obj.push(10);
    } else if(str == "Puerto Rico"){
        obj.push(18.2766);
        obj.push(-66.3350);
        obj.push(8);
    } else if(str == "Virgin Islands"){
        obj.push(18.0001);
        obj.push(-64.8199);
        obj.push(9);
    } else if(str == "Tribal Land"){
        obj.push(39);
        obj.push(-97);
        obj.push(4);
    }
    return obj;
}

function isState(str){
    if(str == "Alaska"){
        return 2;
    } else if(str == "American Samoa"){
        return 60;
    } else if(str == "Nevada"){
        return 32;
    } else if(str == "Arizona"){
        return 4;
    } else if(str == "Wisconsin"){
        return 55;
    } else if(str == "Northern Mariana Islands"){
        return 69;
    } else if(str == "Georgia"){
        return 13;
    } else if(str == "Virgin Islands"){
        return 78;
    } else if(str == "Kansas"){
        return 20;
    } else if(str == "Connecticut"){
        return 9;
    } else if(str == "Indiana"){
        return 18;
    } else if(str == "Maine"){
        return 23;
    } else if(str == "Massachusetts"){
        return 25;
    } else if(str == "Montana"){
        return 30;
    } else if(str == "Maryland"){
        return 24;
    } else if(str == "Arkansas"){
        return 5;
    } else if(str == "Alabama"){
        return 1;
    } else if(str == "Virginia"){
        return 51;
    } else if(str == "Nebraska"){
        return 31;
    } else if(str == "Guam"){
        return 66;
    } else if(str == "Kentucky"){
        return 21;
    } else if(str == "New York"){
        return 36;
    } else if(str == "Colorado"){
        return 8;
    } else if(str == "Vermont"){
        return 50;
    } else if(str == "South Dakota"){
        return 46;
    } else if(str == "Michigan"){
        return 26;
    } else if(str == "Missouri"){
        return 29;
    } else if(str == "Hawaii"){
        return 15;
    } else if(str == "Puerto Rico"){
        return 72;
    } else if(str == "North Carolina"){
        return 37;
    } else if(str == "Rhode Island"){
        return 44;
    } else if(str == "Idaho"){
        return 16;
    } else if(str == "Delaware"){
        return 10;
    } else if(str == "District Of Columbia"){
        return 11;
    } else if(str == "New Hampshire"){
        return 33;
    } else if(str == "Minnesota"){
        return 27;
    } else if(str == "North Dakota"){
        return 38;
    } else if(str == "Oklahoma"){
        return 40;
    } else if(str == "Iowa"){
        return 19;
    } else if(str == "Tennessee"){
        return 47;
    } else if(str == "Florida"){
        return 12;
    } else if(str == "Louisiana"){
        return 22;
    } else if(str == "New Mexico"){
        return 35;
    } else if(str == "Wyoming"){
        return 56;
    } else if(str == "Pennsylvania"){
        return 42;
    } else if(str == "South Carolina"){
        return 45;
    } else if(str == "Utah"){
        return 49;
    } else if(str == "West Virginia"){
        return 54;
    } else if(str == "Washington"){
        return 53;
    } else if(str == "Mississippi"){
        return 28;
    } else if(str == "Oregon"){
        return 41;
    } else if(str == "Illinois"){
        return 17;
    } else if(str == "New Jersey"){
        return 34;
    } else if(str == "California"){
        return 6;
    } else if(str == "Ohio"){
        return 39;
    } else if(str == "Texas"){
        return 48;
    } else if(str == "Tribal Land"){
        return 80;
    }
    return 0;
}

function abbreviationToState(str){
    if(str == "AK"){
        return "Alaska";
    } else if(str == "AS"){
        return "American Samoa";
    } else if(str == "NV"){
        return "Nevada";
    } else if(str == "AZ"){
        return "Arizona";
    } else if(str == "WI"){
        return "Wisconsin";
    } else if(str == "MP"){
        return "Northern Mariana Islands";
    } else if(str == "GA"){
        return "Georgia";
    } else if(str == "VI"){
        return "Virgin Islands";
    } else if(str == "KS"){
        return "Kansas";
    } else if(str == "CT"){
        return "Connecticut";
    } else if(str == "IN"){
        return "Indiana";
    } else if(str == "ME"){
        return "Maine";
    } else if(str == "MA"){
        return "Massachusetts";
    } else if(str == "MT"){
        return "Montana";
    } else if(str == "MD"){
        return "Maryland";
    } else if(str == "AR"){
        return "Arkansas";
    } else if(str == "AL"){
        return "Alabama";
    } else if(str == "VA"){
        return "Virginia";
    } else if(str == "NE"){
        return "Nebraska";
    } else if(str == "GU"){
        return "Guam";
    } else if(str == "KY"){
        return "Kentucky";
    } else if(str == "NY"){
        return "New York";
    } else if(str == "CO"){
        return "Colorado";
    } else if(str == "VT"){
        return "Vermont";
    } else if(str == "SD"){
        return "South Dakota";
    } else if(str == "MI"){
        return "Michigan";
    } else if(str == "MO"){
        return "Missouri";
    } else if(str == "HI"){
        return "Hawaii";
    } else if(str == "PR"){
        return "Puerto Rico";
    } else if(str == "NC"){
        return "North Carolina";
    } else if(str == "RI"){
        return "Rhode Island";
    } else if(str == "ID"){
        return "Idaho";
    } else if(str == "DE"){
        return "Delaware";
    } else if(str == "DC"){
        return "District Of Columbia";
    } else if(str == "NH"){
        return "New Hampshire";
    } else if(str == "MN"){
        return "Minnesota";
    } else if(str == "ND"){
        return "North Dakota";
    } else if(str == "OK"){
        return "Oklahoma";
    } else if(str == "IA"){
        return "Iowa";
    } else if(str == "TN"){
        return "Tennessee";
    } else if(str == "FL"){
        return "Florida";
    } else if(str == "LA"){
        return "Louisiana";
    } else if(str == "NM"){
        return "New Mexico";
    } else if(str == "WY"){
        return "Wyoming";
    } else if(str == "PA"){
        return "Pennsylvania";
    } else if(str == "SC"){
        return "South Carolina";
    } else if(str == "UT"){
        return "Utah";
    } else if(str == "WV"){
        return "West Virginia";
    } else if(str == "WA"){
        return "Washington";
    } else if(str == "MS"){
        return "Mississippi";
    } else if(str == "OR"){
        return "Oregon";
    } else if(str == "IL"){
        return "Illinois";
    } else if(str == "NJ"){
        return "New Jersey";
    } else if(str == "CA"){
        return "California";
    } else if(str == "OH"){
        return "Ohio";
    } else if(str == "TX"){
        return "Texas";
    } else if(str == "TL"){
        return "Tribal Land";
    }
    return "";
}

function stateToAbbreviation(str){
    if(str == "Alaska"){
        return "AK";
    } else if(str == "American Samoa"){
        return "AS";
    } else if(str == "Nevada"){
        return "NV";
    } else if(str == "Arizona"){
        return "AZ";
    } else if(str == "Wisconsin"){
        return "WI";
    } else if(str == "Northern Mariana Islands"){
        return "MP";
    } else if(str == "Georgia"){
        return "GA";
    } else if(str == "Virgin Islands"){
        return "VI";
    } else if(str == "Kansas"){
        return "KS";
    } else if(str == "Connecticut"){
        return "CT";
    } else if(str == "Indiana"){
        return "IN";
    } else if(str == "Maine"){
        return "ME";
    } else if(str == "Massachusetts"){
        return "MA";
    } else if(str == "Montana"){
        return "MT";
    } else if(str == "Maryland"){
        return "MD";
    } else if(str == "Arkansas"){
        return "AR";
    } else if(str == "Alabama"){
        return "AL";
    } else if(str == "Virginia"){
        return "VA";
    } else if(str == "Nebraska"){
        return "NE";
    } else if(str == "Guam"){
        return "GU";
    } else if(str == "Kentucky"){
        return "KY";
    } else if(str == "New York"){
        return "NY";
    } else if(str == "Colorado"){
        return "CO";
    } else if(str == "Vermont"){
        return "VT";
    } else if(str == "South Dakota"){
        return "SD";
    } else if(str == "Michigan"){
        return "MI";
    } else if(str == "Missouri"){
        return "MO";
    } else if(str == "Hawaii"){
        return "HI";
    } else if(str == "Puerto Rico"){
        return "PR";
    } else if(str == "North Carolina"){
        return "NC";
    } else if(str == "Rhode Island"){
        return "RI";
    } else if(str == "Idaho"){
        return "ID";
    } else if(str == "Delaware"){
        return "DE";
    } else if(str == "District Of Columbia"){
        return "DC";
    } else if(str == "New Hampshire"){
        return "NH";
    } else if(str == "Minnesota"){
        return "MN";
    } else if(str == "North Dakota"){
        return "ND";
    } else if(str == "Oklahoma"){
        return "OK";
    } else if(str == "Iowa"){
        return "IA";
    } else if(str == "Tennessee"){
        return "TN";
    } else if(str == "Florida"){
        return "FL";
    } else if(str == "Louisiana"){
        return "LA";
    } else if(str == "New Mexico"){
        return "NM";
    } else if(str == "Wyoming"){
        return "WY";
    } else if(str == "Pennsylvania"){
        return "PA";
    } else if(str == "South Carolina"){
        return "SC";
    } else if(str == "Utah"){
        return "UT";
    } else if(str == "West Virginia"){
        return "WV";
    } else if(str == "Washington"){
        return "WA";
    } else if(str == "Mississippi"){
        return "MS";
    } else if(str == "Oregon"){
        return "OR";
    } else if(str == "Illinois"){
        return "IL";
    } else if(str == "New Jersey"){
        return "NJ";
    } else if(str == "California"){
        return "CA";
    } else if(str == "Ohio"){
        return "OH";
    } else if(str == "Texas"){
        return "TX";
    } else if(str == "Tribal Land"){
        return "TL";
    }
    return "";
}
